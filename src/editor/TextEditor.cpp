...
//TODO add tooltips
    static const std::pair<std::string, std::string> words[] = {
      ...
      {"error", "Raises an error (see §2.3) with @{message} as the error object. This function never returns."},
      ...
      {"abs", "Returns the maximum value between x and -x. (integer/float)"},
      ...
      {"ceil", "Returns the smallest integral value greater than or equal to x."},
      {"cos", "Returns the cosine of x (assumed to be in radians)."},
      {"deg", "Converts the angle x from radians to degrees."},
      {"exp", "Returns the value ex (where e is the base of natural logarithms)."},
      {"floor", "Returns the largest integral value less than or equal to x."},
      {"fmod", "Returns the remainder of the division of x by y that rounds the quotient towards zero. (integer/float)"},
      {"huge", "The float value HUGE_VAL, a value greater than any other numeric value."},
      {"log", "Returns the logarithm of x in the given base. The default for base is e (so that the function returns the natural logarithm of x)."},
      {"max", "Returns the argument with the maximum value, according to the Lua operator <."},
      {"maxinteger", "An integer with the maximum value for an integer."},
      {"min", "Returns the argument with the minimum value, according to the Lua operator <."},
      {"mininteger", "An integer with the minimum value for an integer."},
      {"modf", "Returns the integral part of x and the fractional part of x. Its second result is always a float."},
      {"pi", "The value of π."},
      ...
      {"randomseed", "x and y are joined into a 128-bit seed that is used to reinitialize the pseudo-random generator; equal seeds produce equal sequences of numbers. The default for y is zero."},
      {"sin", "Returns the sine of x (assumed to be in radians)."},
      {"sqrt", "Returns the square root of x."},
      {"tan", "Returns the tangent of x (assumed to be in radians)."},
      {"tointeger", "If the value x is convertible to an integer, returns that integer. Otherwise, returns fail."},
      {"clock", "Returns an approximation of the amount in seconds of CPU time used by the program, as returned by the underlying ISO C function clock."},
      {"date", "Returns a string or a table containing date and time, formatted according to the given string format."},
      {"difftime", "Returns the difference, in seconds, from time t1 to time t2."},
      {"time", "Returns the current time."},
      {"format", "Returns a formatted version of its variable number of arguments following the description given in its first argument, which must be a string. The format string follows the same rules as the ISO C function sprintf."},
      {"gmatch", "Returns an iterator function that, each time it is called, returns the next captures from pattern"},
      ...
      {"lower", "Receives a string and returns a copy of this string with all uppercase letters changed to lowercase."},
      {"match", "Looks for the first match of the pattern in the string s"},
      {"rep", "Returns a string that is the concatenation of n copies of the string s separated by the string sep."},
      ...
      {"upper", "Receives a string and returns a copy of this string with all lowercase letters changed to uppercase."},
      {"concat", "Given a list where all elements are strings or numbers, returns the string list[i]..sep..list[i+1] ··· sep..list[j]"},
      {"insert", "Inserts element value at position pos in list, shifting up the elements list[pos], list[pos+1], ···, list[#list]."},
      {"remove", "Removes from list the element at position pos, returning the value of the removed element."},
      {"sort", "Sorts the list elements in a given order, in-place, from list[1] to list[#list]."},
      {"line", "line( int x1, int y1, int x2, int y2, uint32 col, float thickness = 1.0f )"},
      {"rect", "rect( int p_min_x, int p_min_y, int p_max_x, int p_max_y, uint32 col, float rounding = 0.0f, float thickness = 1.0f )"},
      {"rectfill", "rectfill( int p_min_x, int p_min_y, int p_max_x, int p_max_y, uint32 col, float rounding = 0.0f )"},
      {"rectfillgradient", "rectfillgradient( int p_min_x, int p_min_y, int p_max_x, int p_max_y, uint32 col_upr_left, uint32 col_upr_right, uint32 col_bot_right, uint32 col_bot_left )"},
      {"quad", "quad( int x1, int y1, int x2, int y2,int x3, int y3, int x4, int y4, uint32 col, float thickness = 1.0f )"},
      {"quadfill", "quadfill( int x1, int y1, int x2, int y2,int x3, int y3, int x4, int y4, uint32 col )"},
      {"triangle", "triangle( int x1, int y1, int x2, int y2,int x3, int y3, uint32 col, float thickness = 1.0f )"},
      {"trianglefill", "trianglefill( int x1, int y1, int x2, int y2,int x3, int y3, uint32 col )"},
      {"circle", "circle( int x, int y, float radius, uint32 col, int num_segments = 0, float thickness = 1.0f )"},
      {"circlefill", "circlefill( int x, int y, float radius, uint32 col, int num_segments = 0 )"},
      {"ngon", "ngon( int x, int y, float radius, uint32 col, int num_segments, float thickness = 1.0f )"},
      {"ngonfill", "ngonfill( int x, int y, float radius, uint32 col, int num_segments )"},
      {"text", "text( int x, int y, uint32 col, String text_begin )"},
      {"beziercurve", "beziercurve( int x1, int y1, int x2, int y2,int x3, int y3, int x4, int y4, uint32 col, float thickness, int num_segments = 0 )"},
      {"size", "Returns the size of the screen"},
      {"center", "Returns the center of the screen."},
    };

    for( auto [word, is] : words ) {
      langDef.mIdentifiers.insert( std::make_pair( word, Identifier( is ) ) );
    }
