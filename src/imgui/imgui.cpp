...
static void ImGui::ErrorCheckBeginEndCompareStacksSize( ImGuiWindow *window, bool write ) {
  ...
  {
    int n = window->IDStack.Size;        // Too few or too many PopID()/TreePop()
  ...
  {
    int n = window->DC.GroupStack.Size;  // Too few or too many EndGroup()
  ...
  {
    int n = g.BeginPopupStack.Size;      // Too few or too many EndMenu()/EndPopup()
  ...
  {
    int n = g.ColorModifiers.Size;       // Too few or too many PopStyleColor()
  ...
  {
    int n = g.StyleModifiers.Size;       // Too few or too many PopStyleVar()
  ...
  {
    int n = g.FontStack.Size;            // Too few or too many PopFont()
...
static void ImGui::NavUpdate() {
  ...
  if( g.NavScoringCount > 0 ) {
    IMGUI_DEBUG_LOG( "NavScoringCount %d for '%s' layer %d (Init:%d, Move:%d)\n", g.FrameCount, g.NavScoringCount, g.NavWindow ? g.NavWindow->Name : "NULL", g.NavLayer, g.NavInitRequest || g.NavInitResultId != 0, g.NavMoveRequest );
  }
  ...
  if( g.NavWindow ) {
    ...
    if( 1 ) {
      for( int layer = 0; layer < 2; layer++ ) {
        draw_list->AddRect( g.NavWindow->Pos + g.NavWindow->NavRectRel[layer].Min, g.NavWindow->Pos + g.NavWindow->NavRectRel[layer].Max, IM_COL32( 255, 200, 0, 255 ) );  // [DEBUG]
      }
    }
