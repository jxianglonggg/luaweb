...
struct ImVec2 {
  ...
  float  operator[]( size_t idx ) const    {
    IM_ASSERT( idx <= 1 );  // We very rarely use this [] operator, the assert overhead is fine.
    return ( &x )[idx];
  }
...
struct ImVector {
  ...
  inline int          max_size() const                    {
    return ( ~( unsigned int )0 ) / ( int )sizeof( T );
  }
  ...
  inline void         resize( int new_size, const T &v )    {
    ...
    if( new_size > Size ) for( int n = Size; n < new_size; n++ ) {
...
namespace ImGui {
  ...
  static inline bool  OpenPopupOnItemClick( const char *str_id = NULL, ImGuiMouseButton mb = 1 )          {
    return OpenPopupContextItem( str_id, mb );  // Passing a mouse button to ImGuiPopupFlags is legal
  }
...
struct ImDrawList {
  ...
  inline    void  PathFillConvex( ImU32 col )                                   {
    AddConvexPolyFilled( _Path.Data, _Path.Size, col );  // Note: Anti-aliased filling requires points to be in clockwise order.
    _Path.Size = 0;
  }
...
struct ImDrawData {
  ...
  void Clear()    {
    Valid = false;  // The ImDrawList are owned by ImGuiContext!
    CmdLists = NULL;
